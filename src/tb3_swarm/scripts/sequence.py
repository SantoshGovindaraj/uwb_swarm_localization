...
class MoveBaseSeq():
    ...
    def __init__(self):
        ...
        # Get the Namespaces
        self.robot_lead = "/robot1/"
        rospy.loginfo("robot_ns ->" + self.robot_lead)
        # Reading JSON file
        ...
        self.client_1 = actionlib.SimpleActionClient('/robot1/move_base',MoveBaseAction)
        self.client_2 = actionlib.SimpleActionClient('/robot2/move_base',MoveBaseAction)
        rospy.loginfo("Waiting for move_base action server 1...")
        wait = self.client_1.wait_for_server(rospy.Duration(5.0))
        if not wait:
            rospy.logerr("Action server 1 not available!")
            rospy.signal_shutdown("Action server 1 not available!")
            return
        rospy.loginfo("Connected to move base server 1")
        rospy.loginfo("Starting goals achievements Robot 1 ...")

        wait_2 = self.client_2.wait_for_server(rospy.Duration(5.0))
        if not wait_2:
            rospy.logerr("Action server 2 not available")
            rospy.signal_shutdown("Action Server 2 not available")
            return
        rospy.loginfo("Connected to move base server 2")
        rospy.loginfo("Starting goal achievements for Robot 2 ...")
    ...
    # Reference for terminal status values: http://docs.ros.org/diamondback/api/actionlib_msgs/html/msg/GoalStatus.html
        ...
        if status == 3:
            ...
            if self.goal_cnt< len(self.pose_seq):
                ...
                time.sleep(5)
                rospy.loginfo("Sending goal pose "+str(self.goal_cnt+1)+" to Action Server 1")
                # rospy.loginfo(str(self.pose_seq[self.goal_cnt]))
                self.client_1.send_goal(next_goal, self.done_cb, self.active_cb)

                prev_goal = MoveBaseGoal()
                prev_goal.target_pose.header.frame_id = "map"
                prev_goal.target_pose.header.stamp = rospy.Time.now()
                prev_goal.target_pose.pose = self.pose_seq[self.goal_cnt-1]
                rospy.loginfo("Sending goal pose "+str(self.goal_cont+1)+" to Action Server 2")
                self.clent_2.send_goal(prev_goal)
    ...
    def movebase_client(self):
        ...
        self.client_1.send_goal(goal, self.done_cb, self.active_cb)
        rospy.spin()
